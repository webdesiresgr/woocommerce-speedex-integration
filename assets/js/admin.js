jQuery(document).ready(function ($) {
	...
	$('#speedex_order_fields').on('click', 'input.manually_create_voucher', function (e) {
		...
		$.ajax({
			...
			error: function (jqXHR, textStatus, errorThrown) {
				$('#wc_speedex_cep_vouchers').LoadingOverlay('hide');
				var detailedMessage = wc_speedex_cep_local.ajaxErrorMessage;
				if (errorThrown) {
					detailedMessage += ' (' + errorThrown + ')';
				}
				if ($(currentElementClicked).closest('div').has('.speedex_cep_create_voucher_error_message').length === 0) {
					$(currentElementClicked).closest('div').append($('<label type="text" class="speedex_cep_error_message speedex_cep_create_voucher_error_message" style="display:block; ">' + detailedMessage + '</label>'));
				}
			}
		});
